...
public class SendImageHolder extends BaseViewHolder {
    ...
    public SendImageHolder(Context context, ViewGroup root, BmobIMConversation conversation, OnRecyclerViewListener listener) {
        super(context, root, R.layout.recyclerview_item_sent_image_chat, listener);
    ...
    public void bindData(Object o) {
        ...
        //发送的不是远程图片地址，则取本地地址
        String url = message.getRemoteUrl();
        if (TextUtils.isEmpty(url)) {
            url = message.getLocalPath();
        }
        Picasso.with(mContext).load(url).into(iv_picture);
