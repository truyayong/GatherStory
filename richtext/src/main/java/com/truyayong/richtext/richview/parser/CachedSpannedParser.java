...
import com.truyayong.richtext.richview.spans.ClickableImageSpan;
import com.truyayong.richtext.richview.spans.LongClickableURLSpan;
...
public class CachedSpannedParser {
    ...
    public int parse(SpannableStringBuilder ssb, ImageGetterWrapper imageGetter, RichViewConfig config) {
        boolean cached = isCached(ssb);
        handleClick(ssb, config, cached);
        return handleImage(ssb, imageGetter, config, cached);
    }

    private void handleClick(SpannableStringBuilder ssb, RichViewConfig config, boolean cached) {
        if (cached) {
    ...
    private void resetLinkSpan(SpannableStringBuilder ssb, RichViewConfig config, URLSpan urlSpan) {
    ...
    private int handleImage(SpannableStringBuilder ssb, ImageGetterWrapper imageGetterWrapper, RichViewConfig config, boolean cached) {
        if (cached) {
